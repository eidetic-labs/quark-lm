...
- Validate PyTorch training parity attempt summaries before trust or write so
  ...
  runtime report, readiness, replay gate, and training-case routing must have
  standalone validation. The backend-promotion gate must also have standalone
  validation for schema, check catalog, blocker derivation, boundary state, and
  required future gate catalog. Standalone summary validation must check compact
...
- Any future tokenizer upgrade must be trained only from admitted corpus text;
  ...
  manifest/report artifacts must pass standalone validation before writing,
  before tokenizer benchmarks trust them, and again inside the tokenizer
  candidate guard before self-improvement candidate evidence is trusted.
