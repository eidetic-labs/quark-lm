...
derivation, and required future gate catalog. The next-requirements diagnosis
must also rebuild from the candidate runtime report, candidate, and report.
Written summaries also carry SHA-256 payload hashes for sibling artifacts, and
the persisted corpus summary must match the scalar fixture and candidate
backend corpus hash. Written attempt directories are reloaded through the same
