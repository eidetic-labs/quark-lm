...
class TransformerRoutingRepairTopkBundleEvidenceTests(unittest.TestCase):
    def test_topk_bundle_rejects_pressure_without_branch_response(self) -> None:
        ...
        self.assertFalse(
            by_name["topk_pressure_requires_branch_response"]["passed"]
        )

    def test_topk_bundle_accepts_branch_score_response(self) -> None:
        checks = routing_repair_bundle_checks(
            _metrics(baseline_rank=20.0, final_rank=7.0, final_top3=0.5)
        )
...
def _metrics(
    *,
    ...
    return {
        ...
        "sweep_plan_path": "runs/bundle-c/sweep_plan.json",
        "replay_mixture_report": {"summary": {"passed": True}},
        "replay_mixture_report_path": "runs/bundle-c/replay_mixture_report.json",
        "pretrained_weights": False,
        "pretrained_tokenizer": False,
        "external_embeddings": False,
        "backend": transformer_backend_metadata(seed=17, tokenizer_type="char"),
        "direct_answer": {
            "direct_answer_mode": PROFILE_BALANCED_TOPK_ROUTING_REPAIR_MODE,
