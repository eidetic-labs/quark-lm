...
class CorpusResponder:
    ...
    def answer_prompt(self, prompt: str) -> str:
        ...
        training_data_match = TRAINING_DATA_QUESTION_RE.search(prompt) or TRAINING_DATA_TAG_RE.search(
            prompt
        )
        ...
        glossary_match = GLOSSARY_MEANING_RE.search(prompt) or GLOSSARY_DEFINE_RE.search(prompt)
        if glossary_match:
