...
class TransformerTorchTrainingStateTests(unittest.TestCase):
    ...
    def test_backward_probe_reports_missing_gradients_honestly(self) -> None:
        fixture = _scalar_training_fixture()
        importer = fake_torch_importer(training_runtime=True)
        runtime = torch_runtime_status(importer=importer)
        state = build_torch_training_state(
        ...
        probe = build_torch_training_backward_probe(
            fixture=fixture,
            state=state,
            torch=importer("torch"),
            runtime=runtime,
        ...
        self.assertEqual(probe["status"], "gradients_missing")
        self.assertTrue(probe["loss_matches_initial"])
        self.assertEqual(
            probe["gradient_summary"]["missing_gradient_tensor_count"],
            state["tensor_count"],
        )

    def test_backward_probe_reports_gradient_coverage(self) -> None:
        fixture = _scalar_training_fixture()
        ...
        self.assertEqual(probe["status"], "gradients_available")
        self.assertEqual(
            probe["gradient_summary"]["gradient_tensor_count"],
            state["tensor_count"],
        )
        self.assertEqual(probe["gradient_summary"]["missing_gradient_parameters"], [])
