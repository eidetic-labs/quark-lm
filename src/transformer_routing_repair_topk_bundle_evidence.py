"""Evidence checks for profile-balanced top-k routing repair."""
...
def topk_routing_repair_checks(metrics: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    coverage_preserved = _coverage_surface_present(
        coverage_delta
    ...
    return [
        promotion_check(
            ...
            "Bundle C must record profile-balanced branch batches.",
        ...
        promotion_check(
            ...
            "Bundle C must record representation-separation evidence.",
            {
        ...
        promotion_check(
            ...
            coverage_preserved,
            "Bundle C must reject target-token coverage regressions.",
            coverage_delta,
        ),
        promotion_check(
            "branch_diversity_acceptance_gate",
            diversity_passed,
            "Bundle C accepts only when branch-diversity evidence passes.",
            {"branch_diversity_target": diversity},
        ),
        promotion_check(
            "topk_pressure_requires_branch_response",
            _topk_pressure_configured(direct_answer) and branch_responded,
            (
                "Bundle C must reject top-k movement when target coverage and "
...
def _topk_pressure_configured(direct_answer: dict[str, Any]) -> bool:
    return (
