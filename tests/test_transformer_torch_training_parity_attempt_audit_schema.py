...
class TransformerTorchTrainingParityAttemptAuditSchemaTests(unittest.TestCase):
    def test_validator_accepts_exact_valid_audit_keys(self) -> None:
        validate_torch_training_parity_attempt_audit(_valid_audit())

    def test_validator_accepts_exact_invalid_audit_keys(self) -> None:
        validate_torch_training_parity_attempt_audit(_invalid_audit())

    def test_validator_rejects_extra_valid_audit_key(self) -> None:
        audit = _valid_audit()
        audit["unvalidated_extra_field"] = "drift"

        with self.assertRaisesRegex(ValueError, "audit.keys"):
            validate_torch_training_parity_attempt_audit(audit)

    def test_validator_rejects_extra_invalid_audit_key(self) -> None:
