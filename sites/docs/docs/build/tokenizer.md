...
checkpoint unless explicit paths are supplied. The manifest/report pair must
validate before it is written. Answer-training metrics and `sweep_plan.json`
record the tokenizer type and manifest hash so char and subword screens can be
compared without hidden tokenizer drift.
...
cycle validates the manifest/report pair before writing it, then
`tokenizer_candidate_guard` rechecks the embedded pair and blocks promotion if
the proposal uses pretrained state, imports outside vocabulary, fails
round-trip, or creates protected full-answer tokens.
...
The benchmark is intentionally narrow. It validates the same manifest/report
contract, then proves compression and round-trip behavior; it does not prove the
transformer weights have learned the answers.
