...
summary validator also checks the promotion-gate schema, check catalog, blocker
derivation, required future gate catalog, and compact corpus, runtime,
...
private module paths. Each `next_requirements` summary is a typed artifact with
an explicit kind and schema version, and the public backend surface exposes
that contract for loop consumers. The requirements artifact also has standalone
validation and a public stage catalog so next-action routing can be checked
without validating a full attempt bundle. Stage/action consistency is validated
too, so a well-shaped artifact cannot route a replay blocker through a
readiness or runtime action by mistake. Runtime preflight actions come from a
canonical status-to-action map that the standalone validator also checks.
