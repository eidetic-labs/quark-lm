---
title: Transformer screen history
description: The complete version-by-version screen log and evidence tables for the QuarkLM transformer.
---

# Transformer screen history

This page preserves the complete, version-by-version record of transformer
screens and their evidence tables. It is a reference archive. For how the
transformer works and its current status, start with
[Transformer](./transformer.md).

v0.24 introduced a tiny decoder-only transformer in
`transformer_char_model`.
...
`src/replay_plan.py`. The transformer still emits the same
...
From v0.79 onward, `src/transformer_model.py` owns model,
...
From v0.80 onward, `src/transformer_checkpoint.py` owns
checkpoint payload loading and identity validation, and
`src/transformer_eval.py` owns generic transformer probe
...
It keeps the v0.97 coverage audit but allows coverage-tied preparation moves
when they improve the branch-diversity score. The diagnostic screen restores
the nine accepted source-profile frontier updates while separating three
coverage gains from six coverage-preparation moves, so the self-improvement
ledger can distinguish real coverage recovery from safe setup movement.
...
It keeps the v0.98 preparation path but gives each safe preparation candidate a
small missing-target recovery retry before falling back to the prepared state.
The diagnostic screen accepts six source-profile updates, converts two
...
branch-diversity recovery step after already-safe profile updates. The
diagnostic screen accepts six source-profile updates, runs branch-diversity
...
`src/memory_retrieval.py`. Transformer answer-training runs now
...
`src/memory_consolidation.py`. Transformer answer-training runs
