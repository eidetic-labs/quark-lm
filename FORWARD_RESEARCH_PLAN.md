...
training-capable, the candidate remains `pending` with
...
`training_replay_parity_matched` without promoting a general trainer. When the
...
report now includes this aggregate gate as a required PyTorch check, and a
PyTorch training-candidate kind cannot bypass those report checks through
drifted backend metadata. The gate is status-aware for replay buffer, update,
...
explicit parity proof flags before they can count. It is schema-aware as well:
replay probes must report the expected schema version before they can count.
Finally, it requires replay-control count consistency: planned, executed,
...
preflight reports now have standalone validation for schema, status, check
catalog, summary counts, closed-world boundary, and eligibility flags before
they can feed candidate or parity evidence. The
...
Optional real-runtime PyTorch parity is also covered by a skip-safe integration
...
gate, and training-case routing now have standalone validation. The
...
runtime, candidate, replay-gate, and report summaries now carry SHA-256 hashes
of the full evidence payloads they summarize, written summaries carry SHA-256
payload hashes for sibling artifacts, and the persisted corpus summary must
match the scalar fixture and candidate backend corpus hash. Written attempt
directories are reloaded through the same validation contract before the writer
returns.
...
backend surface exposes the written-attempt file map, hash algorithm, artifact
hash builder, runtime-report hash builder, attempt payload hash builder, loader,
compact summary validator, compact audit-result builder, validator, status
catalog, and evidence-hash key catalog plus the backend-promotion-gate
validator, candidate validator, and candidate key catalog plus the
runtime-report validator, status catalog, check catalog, and evidence-scope
constant so contributors can inspect the persisted
...
- The character tokenizer is still the purity baseline, but any future subword
  ...
  manifest hash before self-improvement candidates, transformer subword screens,
  or tokenizer benchmarks can trust them.
