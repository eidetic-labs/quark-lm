from __future__ import annotations

import json
import sys
import unittest
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))

from support.char_model import char_model_fixture, context_and_target
from support.fake_torch import fake_torch_importer
from transformer_model import OptimizationConfig
from transformer_torch_backend import (
    TORCH_REPLAY_CHECKPOINT_MATCHED_STATUS,
    TORCH_REPLAY_CHECKPOINT_NOT_RUN_STATUS,
    build_torch_replay_buffer_comparison,
    build_torch_replay_checkpoint_compatibility,
    build_torch_replay_final_evaluation,
    build_torch_replay_update_comparison,
    build_torch_training_state,
    torch_runtime_status,
)
from transformer_training_parity import build_scalar_training_parity_fixture


class TransformerTorchReplayCheckpointCompatibilityTests(unittest.TestCase):
    def test_checkpoint_compatibility_matches_after_final_evaluation(self) -> None:
        fixture, torch, runtime = _ready_runtime()
        control = _scalar_matching_replay_control(fixture)
        buffer_comparison = build_torch_replay_buffer_comparison(
            fixture=fixture,
            replay_control_probe=control,
        )
        update_comparison = build_torch_replay_update_comparison(
            fixture=fixture,
            state=_training_state(fixture, torch, runtime),
            torch=torch,
            runtime=runtime,
            replay_control_probe=control,
            buffer_comparison=buffer_comparison,
        )
        final_evaluation = build_torch_replay_final_evaluation(
            fixture=fixture,
            state=_training_state(fixture, torch, runtime),
            torch=torch,
            runtime=runtime,
            replay_control_probe=control,
            buffer_comparison=buffer_comparison,
            update_comparison=update_comparison,
        )

        compatibility = build_torch_replay_checkpoint_compatibility(
            fixture=fixture,
            state=_training_state(fixture, torch, runtime),
            torch=torch,
            runtime=runtime,
            replay_control_probe=control,
            buffer_comparison=buffer_comparison,
            final_evaluation=final_evaluation,
        )

        self.assertEqual(compatibility["status"], TORCH_REPLAY_CHECKPOINT_MATCHED_STATUS)
        self.assertTrue(compatibility["passed"])
        self.assertTrue(compatibility["checkpoint_parity_proven"])
        self.assertFalse(compatibility["promoted_training_backend"])
        self.assertTrue(compatibility["round_trip_loaded"])
        self.assertTrue(compatibility["tokenizer_round_trip"])
        self.assertTrue(compatibility["checkpoint_summary"]["has_tokenizer"])
        self.assertTrue(compatibility["checkpoint_summary"]["has_metadata"])
        self.assertLessEqual(compatibility["loss_abs_diff"], 1e-9)
        self.assertLessEqual(compatibility["max_logit_abs_diff"], 1e-9)
        json.dumps(compatibility)

    def test_checkpoint_compatibility_waits_for_final_evaluation(self) -> None:
        fixture, torch, runtime = _ready_runtime()

        compatibility = build_torch_replay_checkpoint_compatibility(
            fixture=fixture,
            state=_training_state(fixture, torch, runtime),
            torch=torch,
            runtime=runtime,
            replay_control_probe=_scalar_matching_replay_control(fixture),
            buffer_comparison={"passed": True},
            final_evaluation={"status": "replay_final_evaluation_mismatch"},
        )

        self.assertEqual(compatibility["status"], TORCH_REPLAY_CHECKPOINT_NOT_RUN_STATUS)
        self.assertFalse(compatibility["checkpoint_parity_proven"])
        self.assertIn("final evaluation", compatibility["reason"])


def _ready_runtime() -> tuple[dict, object, dict]:
    fixture = _scalar_training_fixture()
    importer = fake_torch_importer(
        training_runtime=True,
        gradient_runtime=True,
    )
    torch = importer("torch")
    runtime = torch_runtime_status(importer=importer)
    return fixture, torch, runtime


def _training_state(fixture: dict, torch: object, runtime: dict) -> dict:
    return build_torch_training_state(
        fixture=fixture,
        torch=torch,
        runtime=runtime,
    )


def _scalar_training_fixture() -> dict:
    tokenizer, ids, config, model = char_model_fixture("abc abc\n", seed=53)
    context, target = context_and_target(ids, config, tokenizer)
    return build_scalar_training_parity_fixture(
        fixture_id="tiny-training-scalar",
        model=model,
        tokenizer=tokenizer,
        context=context,
        target=target,
        optimizer_config=OptimizationConfig(
            optimizer="adamw",
            gradient_accumulation_steps=2,
            warmup_steps=2,
            decay_steps=2,
            min_learning_rate=0.001,
        ),
        learning_rate=0.02,
        steps=2,
        corpus_hash="corpus-hash",
    )


def _scalar_matching_replay_control(fixture: dict) -> dict:
    return {
        "status": "accumulation_replay_control_recorded",
        "case_id": fixture["training_case"]["case_id"],
        "microsteps": [
            {
                "step": record["step"],
                "gradient_snapshot": _snapshot(
                    record["optimizer_gradient_evidence"]["clipped_gradient"][
                        "values"
                    ]
                ),
            }
            for record in fixture["training_case"]["step_records"]
        ],
    }


def _snapshot(values: list[float]) -> dict:
    return {
        "parameters": [
            {
                "name": "flat-gradient",
                "values": list(values),
            }
        ]
    }


if __name__ == "__main__":
    unittest.main()
