...
def add_answer_train_arguments(answer_parser: argparse.ArgumentParser) -> None:
    ...
    answer_parser.add_argument("--answer-all-positions", action=argparse.BooleanOptionalAction, default=True, help="Train every target position per step (dense next-token signal). Default on; --no-answer-all-positions reverts to single-position sampling.")
